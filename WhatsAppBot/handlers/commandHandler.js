...
} = require("./nearbyDisastersHandler");
...
async function setupCommandHandlers(
  ...
  if (currentState) {
    ...
    if (text === "!cancel") {
      ...
      msg.reply(
        formatMessage.info(
          "Current operation cancelled. How can I help you further? Type `!help` for options.",
        ),
      );
      return;
    } else if (text === "!stopmonitoring") {
      handleMonitorDisasters.stopMonitoring(msg, senderId, userStates);
      return;
    }
    ...
    switch (currentState.command) {
      case "login":
        await handleLogin.continueFlow(
          msg,
          senderId,
          text,
          currentState,
      ...
      case "report_emergency":
        await handleEmergencyReport.continueFlow(
          msg,
          senderId,
          text,
          currentState,
      ...
      case "nearby_disasters":
        await handleNearbyDisasters.continueFlow(
          msg,
          senderId,
          text,
          currentState,
        ...
        break;
      case "monitor_disasters":
        await handleMonitorDisasters.continueFlow(
          msg,
          senderId,
          text,
          currentState,
      ...
      default:
        msg.reply(
          formatMessage.error(
            "An unexpected error occurred in your conversation flow. Please try starting a new command with `!help`.",
          ),
        );
        userStates.delete(senderId);
    }
  ...
  switch (text) {
      ...
      if (!userTokens.has(senderId)) {
        msg.reply(
          formatMessage.warning(
            "You need to be logged in to report an emergency. Type `!login` to proceed.",
          ),
        );
        return;
      }
    ...
    case "!monitordisasters":
      await handleMonitorDisasters.startFlow(msg, senderId, userStates);
      break;
    ...
    case "!logout":
      if (userTokens.has(senderId)) {
        ...
        userStates.delete(senderId);
        msg.reply(
          formatMessage.success("You have been successfully logged out."),
        );
      } else {
...
async function handleProfile(msg, senderId, userTokens) {
  if (!userTokens.has(senderId)) {
    msg.reply(
      formatMessage.warning(
        "You need to be logged in to view your profile. Type `!login` to proceed.",
      ),
    );
    return;
  }
  ...
  if (response.success) {
    ...
    msg.reply(
      formatMessage.error(`Failed to fetch profile: ${response.error}`),
    );
    ...
    if (response.status === 401) {
      ...
      msg.reply(
        formatMessage.warning(
          "Your session has expired. Please log in again using `!login`.",
        ),
      );
    }
  }
...
async function handleDashboard(msg, senderId, userTokens) {
  if (!userTokens.has(senderId)) {
    msg.reply(
      formatMessage.warning(
        "You need to be logged in to view your dashboard. Type `!login` to proceed.",
      ),
    );
    return;
  }
  ...
  if (response.success) {
    ...
    if (dashboard && Object.keys(dashboard).length > 0) {
      ...
      msg.reply(
        formatMessage.info(
          "Dashboard accessed successfully! No specific data to display.",
        ),
      );
    }
  } else {
    msg.reply(
      formatMessage.error(`Failed to access dashboard: ${response.error}`),
    );
