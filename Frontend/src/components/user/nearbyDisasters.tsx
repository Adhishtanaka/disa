        ...
        <div className="bg-gray-800/30 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-8 shadow-2xl hover:bg-gray-800/40 transition-all duration-300 hover:border-gray-600/50 hover:shadow-blue-500/10">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-semibold text-white flex items-center gap-3">
                    ...
                    Nearby Disasters
                    <span className="ml-2 px-3 py-1 bg-blue-500/20 text-blue-300 rounded-full text-sm font-medium border border-blue-500/30">
                        Live Updates
                    </span>
                </h2>
                <button
                    ...
                    disabled={loading}
                    className="p-3 border border-gray-600/50 bg-gray-900/50 rounded-xl hover:bg-gray-800/70 hover:border-gray-500/50 disabled:opacity-50 transition-all duration-200 backdrop-blur-sm"
                >
            ...
            {loading ? (
                <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className="p-6 border border-gray-600/30 bg-gray-900/30 rounded-xl animate-pulse backdrop-blur-sm">
                ...
                <div className="space-y-4">
                    {disasters.map((disaster, index) => (
                        <Link
                            ...
                            key={index}
                            className={`block p-6 border rounded-xl hover:shadow-xl transition-all duration-300 backdrop-blur-sm hover:border-gray-500/50 transform hover:-translate-y-1 ${getUrgencyColor(disaster.urgency_level)}`}
                        >
                            <div className="flex items-start justify-between mb-3">
                                <h3 className="font-semibold text-lg text-white capitalize flex items-center">
                                    <div className="w-3 h-3 rounded-full mr-3 animate-pulse bg-current"></div>
                                    {disaster.emergency_type} Emergency
                                </h3>
                                <span className={`text-xs px-3 py-1 rounded-full font-medium border ${disaster.urgency_level === 'high' ? 'bg-red-500/20 text-red-400 border-red-500/30' :
                                        disaster.urgency_level === 'medium' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' :
                                            'bg-green-500/20 text-green-400 border-green-500/30'
                                    }`}>
                                    {disaster.urgency_level ? disaster.urgency_level.toUpperCase() : 'UNKNOWN'}
                                </span>
                            </div>
                            <p className="text-sm text-gray-300 mb-4 line-clamp-2">{disaster.situation}</p>
                            <div className="flex items-center justify-between text-xs">
                                <div className="flex items-center">
                                    <div className={`w-2 h-2 rounded-full mr-2 ${disaster.status === 'active' ? 'bg-green-500' : 'bg-gray-500'}`}></div>
                                    <span className={`font-medium ${disaster.status === 'active' ? 'text-green-400' : 'text-gray-400'}`}>
